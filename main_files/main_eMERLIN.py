...
if __name__ == '__main__':
    ...
    clean_imager = Clean(inputvis=selfcal_vis, output=output, niter=10000, M=1024, N=1024, cell="0.02arcsec",
                         stokes="I", datacolumn="corrected", robust=2.0, scales=[0, 3, 5, 10 ,15, 20, 30, 40, 50, 80, 100], specmode="mfs", deconvolver="multiscale", gridder="standard",
                         pbcor=False, savemodel=True, nsigma=3.0, interactive=False, cycleniter=100, usemask='user', mask=output+"_ph2.mask")
    ...
    apcal = AmpPhasecal(minsnr=3.0, solint=solint_ap, varchange=varchange_ap, combine="spw", input_caltable="pcal1", applymode="calonly", Imager=clean_imager, uvrange=">60klambda", **shared_vars_dict)
