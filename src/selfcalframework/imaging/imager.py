...
class Imager(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        Parameters
        ----------
        inputvis : Absolute path to the input Measurement Set file
        output : Absolute path to the output
        cell : Cellsize - example: cell=[’0.5arcsec,’0.5arcsec’] or
        ...
        weighting : Weighting scheme (natural,uniform,briggs,superuniform,radial). During gridding of the dirty
        ...
        stokes : Stokes planes to reconstruct
        phase_center : Phase center of the image
        data_column : Data column to use for image synthesis
        M : Horizontal image size
        N : Vertical image size
        niter : Number of iterations
        noise_pixels : Pixels where to calculate the noise on the residual image. Default is None, and it means to
        calculate the RMS on the whole image
        save_model : Whether to save the model column or not
        verbose : Whether to use verbose option for imagers
        """
    ...
    def _calculate_statistics_fits(
        ...
        Parameters
        ----------
        signal_fits_name : Absolute path to the restored FITS image
        residual_fits_name : Absolute path to the residual FITS image
        stdv_pixels : Pixels where to calculate the RMS
        """
    ...
    def _calculate_statistics_msimage(
        ...
        Parameters
        ----------
        signal_ms_name : Absolute path to the restored CASA image
        residual_ms_name : Absolute path to the residual CASA image
        stdv_pixels : Pixels where to calculate the RMS
        """
        if stdv_pixels is None:
