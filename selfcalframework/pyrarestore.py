...
model_fits = sys.argv[2]
file_restored = sys.argv[3]
file_residuals = sys.argv[4]
weighting = sys.argv[5]
robustparam = float(sys.argv[6])
...
if re.search('Natural',weighting,re.IGNORECASE):
    ...
    fits_io.write(dirty_images_natural[0].data, output_name=file_residuals)
    ...
    fits_io.write(dirty_images_robust[0].data, output_name=file_residuals)
    ...
    fits_io.write(dirty_images_uniform[0].data, output_name=file_residuals)
...
hdu_resid=fits.open(file_residuals)
...
hdu_resid.writeto(file_restored,overwrite=True)
